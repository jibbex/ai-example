export declare type JsonStringify = (obj: object) => string;
/**
...
function jsonStringify(obj: object): string {
     ...
     */
    function createObjAsStr(obj: object, calls: number): { result: string, calls: number } {
        if (MAX_CALLS <= calls) {
            throw new Error('too many functions on call stack');
        }
    
        const isArray = Array.isArray(obj);
        let result: string = isArray ? '[' : '{';
        ...
        for (const [key, val] of Object.entries(obj)) {
            ...
            switch (typeof val) {
                ...
                case 'number':
                    ...
                    break;
                case 'object': {
                    const res = createObjAsStr(val as object, calls++);
                    calls = res.calls;
                    result += res.result;
                    break;
                }
